...
| 이름 | 필수 | 타입 | 설명 |
| --- | --- | --- | --- |
| `nx` | ✅ | number | 기상청 격자 X. 유한수가 아니면 400 |
| `ny` | ✅ | number | 기상청 격자 Y. 유한수가 아니면 400 |
...
- `alert`: 현재 항상 `null`. (폭염특보·낙뢰 배너용 예약 필드. `{ "type": string, "text": string }` 형태가 들어올 자리예요.)
...
| HTTP | `error` | 상황 |
| --- | --- | --- |
| 400 | `INVALID_PARAM` | `nx`/`ny`가 유한수가 아니거나 `profile`이 셋 중 하나가 아님 |
