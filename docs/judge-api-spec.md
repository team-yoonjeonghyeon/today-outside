...
좌표를 행정구역으로 바꿔줘요 (카카오 로컬 API 역지오코딩). 프론트가 GPS로 **정확한 지역명 라벨**을 뽑을 때 써요. 중심점 최근접 방식은 가장자리 동네를 이웃 구로 잘못 잡는데(공덕동→서대문구), 이건 경계 기반이라 정확해요.
...
성공(200):
```jsonc
{ "sido": "서울특별시", "sigungu": "마포구", "dong": "공덕동", "nx": 56, "ny": 126 }
```
- `nx`/`ny`는 워커가 같은 좌표로 계산한 격자예요.
- 실패 시 **502 `REGION_UNAVAILABLE`** (카카오 키 미설정·장애·미매칭). → 프론트는 이때 **기존 중심점 방식(`findNearestRegion`)으로 폴백**하세요. 날씨용 `nx`/`ny`는 프론트가 `toGrid`로 뽑으니 영향 없어요.
